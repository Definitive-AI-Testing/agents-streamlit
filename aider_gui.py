...
class GUI:
    ...
    async def start_server(self):
        try:
            await self.sio.connect(server, auth=Def_API_KEY) #{'token': 'my-token'}
    ...
    def agent_run(self, prompt):
        ...
        print("complete")
        try:
            data = {"Authorization": f"{Def_API_KEY}", "sessionid": session_id, "prompt": self.msgs}  
            self.send_data(data)
        except:
            ()
    ...
    def agent(self,prompt):
        try:
            ...
            chat_response = requests.post(url=server + "/api/aider", headers=headers, json={"prompt": prompt,})
